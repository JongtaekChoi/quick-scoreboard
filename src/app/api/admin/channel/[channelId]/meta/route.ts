...
import { getAccountInfo, isEditAuthorized, validateManagerAgainstDb } from '@/lib/channelSession'
...
export async function GET(_: Request, { params }: { params: Promise<{ channelId: string }> }) {
  ...
  if (account?.role === 'player') {
    return NextResponse.json({ id: channel.id, name: channel.name, slug: channel.slug, role: 'player', managerTeamId: account.teamId })
  }

  const canEdit = await isEditAuthorized(channel.slug, channel.edit_session_version)
  if (canEdit && account?.role === 'manager') {
