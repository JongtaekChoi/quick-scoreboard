-- quick-scoreboard full schema
-- Consolidated from db/schema.sql (pre-deploy)
...
create table if not exists goal_events (
  ...
  scorer_player_id uuid null references team_players(id) on delete set null,
  assist_name text null,
  assist_player_id uuid null references team_players(id) on delete set null,
...
create index if not exists goal_events_scorer_player_idx on goal_events (scorer_player_id) where scorer_player_id is not null;
create index if not exists goal_events_assist_player_idx on goal_events (assist_player_id) where assist_player_id is not null;
