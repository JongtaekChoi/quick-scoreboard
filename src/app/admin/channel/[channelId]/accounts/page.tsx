...
async function upsertAccount(formData: FormData) {
  ...
  const role = String(formData.get('role') || '') as 'admin' | 'editor' | 'manager'
...
async function toggleAccountActive(formData: FormData) {
  ...
  if (!next) {
    const { data: current } = await supabase
      ...
      .eq('id', accountId)
      .maybeSingle<{ id: string; role: 'admin' | 'editor' | 'manager'; is_active: boolean }>()
  ...
  await supabase.from('channel_accounts').update({ is_active: next }).eq('id', accountId)
...
export default async function AdminAccountsPage({
  ...
  const [{ data: accounts }, { data: teams }] = await Promise.all([
    supabase
      ...
      .returns<AccountRow[]>(),
    supabase.from('teams').select('id,name').eq('channel_id', channel.id).order('name', { ascending: true }).returns<Team[]>(),
  ])
    ...
    <main className="min-h-screen p-4 md:p-6 bg-white">
      <section className="max-w-5xl mx-auto space-y-5">
        ...
        <section className="rounded border p-4 space-y-2">
          ...
          <p className="text-xs text-gray-500">동일 로그인 ID가 있으면 비밀번호/권한/팀이 갱신됩니다.</p>
          <form action={upsertAccount} className="grid md:grid-cols-5 gap-2">
            <input type="hidden" name="channelId" value={channel.id} />
            <select className="rounded border px-2 py-1.5 text-sm" name="role" required>
              <option value="editor">골입력자(editor)</option>
              <option value="manager">팀관리자(manager)</option>
              <option value="admin">어드민(admin)</option>
            </select>
            <input className="rounded border px-2 py-1.5 text-sm" name="login_id" placeholder="로그인 ID" required />
            <input className="rounded border px-2 py-1.5 text-sm" name="password" type="password" placeholder="비밀번호" required />
            <select className="rounded border px-2 py-1.5 text-sm" name="team_id" defaultValue="">
              <option value="">팀 미지정 (admin/editor)</option>
              {(teams ?? []).map((t) => (
        ...
        <section className="space-y-2">
          ...
          {(accounts ?? []).length === 0 ? (
            ...
            <ul className="space-y-1">
              {(accounts ?? []).map((a) => (
                <li key={a.id} className="rounded border px-3 py-2 flex items-center justify-between gap-2 text-sm">
                  <div>
                    <span className="font-medium">{a.login_id}</span>
                    <span className="ml-2 text-xs text-gray-600">[{a.role}]</span>
                    {a.team_id ? <span className="ml-2 text-xs text-gray-500">팀: {teamNameById.get(a.team_id) ?? a.team_id}</span> : null}
                    {!a.is_active ? <span className="ml-2 text-xs text-gray-400">(비활성)</span> : null}
                  </div>
                  <form action={toggleAccountActive}>
                    <input type="hidden" name="channelId" value={channel.id} />
                    <input type="hidden" name="accountId" value={a.id} />
                    <input type="hidden" name="next" value={a.is_active ? '0' : '1'} />
                    <button className="text-xs underline" type="submit">{a.is_active ? '비활성화' : '활성화'}</button>
                  </form>
                </li>
              ))}
            </ul>
